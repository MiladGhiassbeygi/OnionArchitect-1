...
namespace Framework.Infra.Data
{
    ...
    {
        public static async Task<PagedList<TSource>> CreatePagedListAsync<TSource>(IQueryable<TSource> source, int pageNumber, int pageSize, CancellationToken cancellationToken=default(CancellationToken))
        {
        ...
        public static async Task<PagedList<TDestination>> CreatePagedListAsync<TSource,TDestination>(IQueryable<TSource> source, int pageNumber, int pageSize, IMapper mapper,CancellationToken cancellationToken=default(CancellationToken))
        {
            var count =await source.CountAsync(cancellationToken);
